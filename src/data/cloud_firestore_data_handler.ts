...
export class CloudFirestoreDataHandler implements DataHandler {
  ...
  async createOrUpdateAuthInfo(
    ...
    if (snapshot.empty) {
      const refreshToken = crypto.randomBytes(512).toString("base64url");
      const data = {
        user_id: userId,
        client_id: clientId,
        scope: scope
  ...
  async validateClient(
    ...
    if (client.exists) {
      ...
      return (
        client.get(`grant_type.${grantType}`) &&
  ...
  async validateClientForAuthorization(
    clientId: string,
    responseType: string
  ): Promise<boolean> {
    const db = admin.firestore();
  ...
  async getAuthInfoByRefreshToken(
    refreshToken: string
  ): Promise<AuthInfo | undefined> {
